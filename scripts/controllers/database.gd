...
func set_next(table_name:String, column:String, set_to, name:String="") -> bool:
    var condition = "year="+str(year+1)
    if name != "":
        condition += "AND name='"+name+"'"
    db.open_db()
    var success:bool=db.update_rows(table_name,condition,{column: str(set_to)})
    db.close_db()
    return success
...
func move_person(name: String, on_farm: bool) -> int:
    db.open_db()
    var row = db.select_rows(FAMILY_TABLE,"name='"+name+"'",["id","on_farm"])
    assert(row.size() == 1)
