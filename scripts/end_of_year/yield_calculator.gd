...
func end_of_year(event: EventResource) -> void:
    ...
    # get all blocks
    var crop_b_dicts: Array = db.get_blocks_and_resources("crop IS NOT NULL", ["crop","structural_measure","irrigation"])
    var f_dict: Dictionary = db.get_current_fertility()[0]
    for b_dict in crop_b_dicts:
        db.add_summary(db.CROP_SUM_TABLE, calculate_yield(b_dict, f_dict, event))
